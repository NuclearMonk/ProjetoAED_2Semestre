...
void        PQ_FixDown(heap_t* heap, int index);
...
void        PQ_FixUp(heap_t* heap, int index){
    while(index>0 && heap->_CompareFunction(heap->_array[index],heap->_array[(index-1)/2])){ /*Iterativamente troca cada dilho com o seu pai se o pai tiver pior prioridade que o filho*/
...
void        PQ_FixDown(heap_t* heap, int index){
    ...
    while(2*index< heap->_size-1){
        ...
        if(heap->_CompareFunction(heap->_array[index],heap->_array[child]))break; /*Verificada a condicao de acerco, logo podemos sair*/
        SWAP(index, child);
        index=child;
    }
}
